...
if [ $stage -le 2 ]; then
  ...
  # Make a reverberated version of the voxcelebing data.  Note that we don't add any
  # additive noise here.
...
if [ $stage -le 3 ]; then
  ...
  # Combine the clean and augmented voxcelebing data.  This is now roughly
  # double the size of the original clean list.
  utils/combine_data.sh data/voxceleb_combined data/voxceleb_reverb data/voxceleb
fi

# Now we prepare the features to generate examples for xvector voxcelebing.
if [ $stage -le 4 ]; then
  # This script applies CMVN and removes nonspeech frames.  Note that this is somewhat
  # wasteful, as it roughly doubles the amount of voxcelebing data on disk.  After
